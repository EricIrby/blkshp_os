...
def _filter_rows_by_permission(
    rows: list[dict[str, Any]], user: str, permission_flag: str = "can_read"
) -> list[dict[str, Any]]:
    if not rows:
        return []
...
def _validate_user_can_modify_product(
    doc: Document, user: str, permission_flag: str
) -> None:
    user = _get_user(user)

    if not frappe.has_permission(
        "Product", permission_flag.replace("can_", ""), doc=doc, user=user
    ):
        frappe.throw(
            _("User {0} lacks {1} permission for Product.").format(
                user, permission_flag
            ),
    ...
    departments = {
        row.department for row in doc.get("departments", []) if row.department
    }
    if not departments:
        return
    ...
    for department in departments:
        if not has_department_permission(user, department, permission_flag):
            frappe.throw(
                _("User {0} lacks {1} permission for Department {2}.").format(
                    user, permission_flag, department
                ),
                frappe.PermissionError,
            )
...
def update_product(
    name: str, data: dict[str, Any], user: str | None = None
) -> dict[str, Any]:
...
def user_can_access_product(
    product: Document | str, user: str | None = None, permission_flag: str = "can_read"
) -> bool:
...
def convert_quantity(
    ...
    if from_unit and to_unit:
        ...
        if not to_unit:
            frappe.throw(
                _("Target unit is required when converting from primary unit.")
            )
...
def get_purchase_units(
    product: str, vendor: str | None = None, user: str | None = None
) -> list[dict[str, Any]]:
