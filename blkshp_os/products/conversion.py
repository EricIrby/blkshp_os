...
import frappe
from frappe import _  # type: ignore[attr-defined]
...
def _load_product_data(product: str | ProductConversionData) -> ProductConversionData:
    ...
    if isinstance(product, str):
        ...
        raise ValueError(
            _(
                "Product must be a string (name), Document, or dict (data), got {0}"
            ).format(type(product))
        )
