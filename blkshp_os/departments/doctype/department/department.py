...
class Department(Document):
    ...
    def autoname(self) -> None:
        ...
        if not code:
            raise frappe.ValidationError(
                _("Department Code is required to name the record.")
            )

        company = (self.company or "").strip()
        if not company:
            raise frappe.ValidationError(
                _("Company is required before naming the department.")
            )
        ...
        if not company_code:
            raise frappe.ValidationError(
                _("Company Code is required to name the department.")
            )
    ...
    def _validate_department_code_uniqueness(self) -> None:
        ...
        if frappe.db.exists("Department", filters):
            raise frappe.ValidationError(
                _("Department Code {0} already exists for company {1}.").format(
                    code, self.company
                )
            )
    ...
    def _validate_parent_department(self) -> None:
        ...
        if self.parent_department == self.name:
            raise frappe.ValidationError(
                _("Parent Department cannot be the same as the department.")
            )

        parent_company = frappe.db.get_value(
            "Department", self.parent_department, "company"
        )
    ...
    def _validate_no_circular_reference(self, parent: str) -> None:
        ...
        while current:
            if current in seen:
                raise frappe.ValidationError(
                    _("Circular department hierarchy detected.")
                )
            seen.add(current)
        ...
        for key, expected_type in SETTINGS_TYPE_MAP.items():
            ...
            if expected_type is list:
                ...
                if not isinstance(value, dict):
                    raise frappe.ValidationError(
                        _("Settings key {0} must be a dictionary.").format(
                            frappe.bold(key)
                        )
                    )
...
def get_accessible_departments(
    user: str, permission_flag: str = "can_read"
) -> list[str]:
    """Return list of departments that user can access with given permission flag."""
    return permission_service.get_accessible_departments(
        user, permission_flag=permission_flag
    )
...
def apply_department_defaults(product: str, department: str) -> None:
    ...
    for row in getattr(product_doc, "departments", []):
        ...
        if hasattr(row, "default_gl_code") and not getattr(
            row, "default_gl_code", None
        ):
