...
class ProductDepartment(Document):
	...
	def _ensure_unique_department_assignment(self) -> None:
		...
		if duplicate:
			raise frappe.ValidationError(
				_("Department {0} is already assigned to this product.").format(self.department)
			)
	...
	def _ensure_single_primary_department(self) -> None:
		...
		primary_count = sum(1 for row in parent_doc.departments if row.is_primary and row.name != self.name)
		if primary_count:
			raise frappe.ValidationError(_("Only one department can be marked as primary per product."))
	...
	def _validate_numeric_fields(self) -> None:
		for field in NUMERIC_FIELDS:
			...
			if value < 0:
				raise frappe.ValidationError(
					_("Field {0} cannot be negative for department {1}.").format(frappe.bold(field), self.department)
				)
	...
	def _apply_department_defaults(self) -> None:
		...
		default_storage_area = frappe.db.get_value("Department", self.department, "default_storage_area")
