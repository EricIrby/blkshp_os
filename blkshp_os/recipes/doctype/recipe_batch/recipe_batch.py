...
class RecipeBatch(Document):
    ...
    def _apply_defaults_from_recipe(self, recipe) -> None:
        ...
        if not self.produced_unit:
            self.produced_unit = (
                recipe.yield_unit or self._get_output_product_primary_unit(recipe)
            )
    ...
    def _ensure_recipe_yield(self, recipe) -> None:
        ...
        if yield_quantity <= 0:
            frappe.throw(
                _("Recipe {0} must have a positive yield before batching.").format(
                    recipe.name
                )
            )
        if recipe.is_prep_item and not recipe.output_product:
            frappe.throw(
                _("Recipe {0} must define an output product to create batches.").format(
                    recipe.name
                )
            )
    ...
    def _recalculate_ingredient_metrics(self, recipe) -> None:
        ...
        for row in self.ingredients or []:
            ...
            row.variance = (row.base_quantity_used or 0) - (
                row.expected_base_quantity or 0
            )
    ...
    def _convert_row_to_primary_units(
        ...
        if recipe_row.ingredient_type == "Recipe":
            ...
            if not subrecipe.output_product:
                frappe.throw(
                    _(
                        "Subrecipe {0} requires an output product before it can be consumed."
                    ).format(subrecipe.name)
                )
            conversion_unit = (
                unit
                or subrecipe.yield_unit
                or self._get_output_product_primary_unit(subrecipe)
            )
        ...
        frappe.throw(
            _("Unsupported ingredient type: {0}").format(recipe_row.ingredient_type)
        )
        return 0.0
    ...
    def _post_inventory_movements(self) -> None:
        ...
        for row in self.ingredients or []:
            ...
            target_product, quantity_primary = (
                self._resolve_target_product_and_quantity(
                    source_row, row.quantity_used, row.unit
                )
            )
            if quantity_primary:
    ...
    def _resolve_target_product_and_quantity(
        ...
        if recipe_row.ingredient_type == "Product":
            ...
            quantity_primary = self._convert_product_quantity(
                product, float(quantity or 0), unit or recipe_row.unit
            )
        ...
        if recipe_row.ingredient_type == "Recipe":
            ...
            if not subrecipe.output_product:
                frappe.throw(
                    _(
                        "Subrecipe {0} must define an output product before batching."
                    ).format(subrecipe.name)
                )
        ...
        frappe.throw(
            _("Unsupported ingredient type for batching: {0}").format(
                recipe_row.ingredient_type
            )
        )
        return "", 0.0
