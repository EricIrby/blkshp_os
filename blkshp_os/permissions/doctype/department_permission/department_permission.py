...
from blkshp_os.permissions.service import get_permission_flags as service_get_permission_flags
...
def run_department_permission_validation(doc: Document, method: str | None = None) -> None:
...
def validate_user_department_permissions(user_doc: Document, method: str | None = None) -> None:
...
def _ensure_department_exists_and_active(doc: Document) -> None:
	...
	if is_active is None:
		raise frappe.ValidationError(_("Department {0} does not exist.").format(doc.department))
	if not cint(is_active):
		raise frappe.ValidationError(_("Department {0} is inactive.").format(doc.department))
	...
	if user_company and user_company != department_company:
		raise frappe.ValidationError(
			_("Department {0} belongs to company {1}, which does not match user company {2}.").format(
				doc.department, department_company, user_company
			)
		)
