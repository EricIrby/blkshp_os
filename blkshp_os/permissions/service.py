...
def validate_permission_flag(permission_flag: str) -> str:
    ...
    if permission_flag not in PERMISSION_FLAGS:
        raise frappe.ValidationError(
            _("Unsupported permission flag: {0}").format(permission_flag)
        )
...
def get_user_subscription_context(user: str | None, *, refresh: bool = False):
    ...
    return get_subscription_context(
        company=get_user_company(user), use_cache=not refresh
    )


def user_has_module_access(
    user: str | None, module_key: str, *, refresh: bool = False
) -> bool:
...
def user_has_feature(
    user: str | None, feature_key: str, *, refresh: bool = False
) -> bool:
...
def get_department_permission_clause(
