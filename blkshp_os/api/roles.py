...
@frappe.whitelist()
def get_permissions_by_category(category: str | None = None) -> dict[str, list[dict[str, Any]]] | list[dict[str, Any]]:
	"""
...
def get_user_permissions(user: str | None = None) -> dict[str, Any]:
	...
	if user != frappe.session.user and not frappe.has_role("System Manager"):
		frappe.throw(_("You do not have permission to view other users' permissions"), frappe.PermissionError)
...
def check_permission(permission_code: str, user: str | None = None) -> dict[str, Any]:
	...
	if has_perm:
		...
		if permission_code in user_perms:
			result["granted_by_roles"] = [p["role"] for p in user_perms[permission_code]]
...
def create_custom_role(
	...
	if not frappe.has_role("System Manager"):
		frappe.throw(_("You do not have permission to create roles"), frappe.PermissionError)
...
def update_role_permissions(
	...
	if not frappe.has_role("System Manager"):
		frappe.throw(_("You do not have permission to modify roles"), frappe.PermissionError)
...
def search_permissions(query: str) -> list[dict[str, Any]]:
	...
	for perm in ALL_PERMISSIONS:
		if (query_lower in perm["name"].lower() or 
			query_lower in perm["description"].lower() or
			query_lower in perm["code"].lower()):
			results.append({
				"code": perm["code"],
				"name": perm["name"],
...
def bulk_assign_permissions(
	...
	return {
		...
		"skipped": skipped,
		"invalid": invalid,
		"message": _("Added {0} permission(s), skipped {1}, invalid {2}").format(added, skipped, len(invalid))
	}


@frappe.whitelist()
def clone_role(source_role: str, new_role_name: str, description: str | None = None) -> dict[str, Any]:
	"""
