...
def get_department_details(department: str) -> dict[str, Any]:
	...
	if not has_department_permission(frappe.session.user, department, "can_read"):
		frappe.throw(_("You do not have permission to view this department"), frappe.PermissionError)
...
@frappe.whitelist()
def get_accessible_departments(permission_flag: str = "can_read") -> list[dict[str, Any]]:
	"""
...
def assign_products_to_department(
	...
	if not has_department_permission(frappe.session.user, department, "can_write"):
		frappe.throw(_("You do not have permission to modify this department"), frappe.PermissionError)
	...
	for product in products:
		try:
			...
			if not existing:
				product_doc.append("departments", {
					"department": department,
...
@frappe.whitelist()
def update_department_settings(department: str, settings: dict[str, Any]) -> dict[str, Any]:
	"""
	...
	if dept_doc.settings:
		existing_settings = json.loads(dept_doc.settings) if isinstance(dept_doc.settings, str) else dept_doc.settings
...
def get_department_statistics(department: str) -> dict[str, Any]:
	...
	child_count = frappe.db.count("Department", filters={"parent_department": department})
