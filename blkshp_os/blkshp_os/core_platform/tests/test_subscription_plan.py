...
class TestSubscriptionPlanFixtures(FrappeTestCase):
    ...
    def test_foundation_plan_fixture_present(self) -> None:
        ...
        self.assertIsNotNone(
            plan_name, "Subscription Plan 'FOUNDATION' should exist as a fixture"
        )
    ...
    def test_default_feature_toggles_registered(self) -> None:
        feature_keys = {
            row.feature_key
            for row in frappe.get_all("Feature Toggle", fields=["feature_key"])
        }
    ...
    def _assert_dependencies_valid(
        self, module_key: str, dependencies: Iterable[str]
    ) -> None:
        dependency_set = set(dependencies)
        self.assertNotIn(
            module_key, dependency_set, f"Module {module_key} cannot depend on itself"
        )
        self.assertTrue(
            dependency_set.issubset(EXPECTED_MODULE_KEYS),
