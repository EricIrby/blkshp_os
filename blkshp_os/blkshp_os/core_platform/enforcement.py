...
def _log_access_denial(
	...
	try:
		...
		frappe.log_error(
			...
			message=f"Failed to log access for user {user}: {str(e)}",
		)
	...
	def check_access(checked_user: str | None = None) -> None:
		...
		if permission_service._user_bypasses_subscription_gates(checked_user):
			...
			if log_denial:
				...
				bypass_reason = ", ".join(
					role for role in permission_service.SUBSCRIPTION_BYPASS_ROLES
...
def enforce_module_access_for_doctype(doc, method=None, module_key: str | None = None):
	...
	if not module_key:
		...
		module_key = frappe.db.get_value(
			"DocType", doc.doctype, "module"
		)
		if not module_key:
			frappe.log_error(
...
def enforce_feature_access_for_doctype(doc, method=None, feature_key: str | None = None):
