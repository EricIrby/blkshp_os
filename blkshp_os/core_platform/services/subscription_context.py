    ...
    if plan_code:
        ...
        if plan_state:
            _apply_feature_overrides(
                feature_states, plan_state.default_feature_overrides
            )
...
) -> dict[str, ModuleActivationState]:
    rows = frappe.get_all(
        "Module Activation",
...
def _apply_feature_overrides(
    target: MutableMapping[str, FeatureValue], overrides: Mapping[str, FeatureValue]
) -> None:
