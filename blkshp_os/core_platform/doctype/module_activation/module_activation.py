...
class ModuleActivation(Document):
    ...
    def _validate_plan_module_uniqueness(self) -> None:
        ...
        if duplicate:
            raise frappe.ValidationError(
                _("Module {0} is already defined for subscription plan {1}.").format(
                    self.module_key, self.plan
                )
            )
    ...
    def _validate_dependencies(self) -> None:
        ...
        dependencies = [
            key.strip().lower() for key in self.depends_on.split(",") if key.strip()
        ]
    ...
    def _validate_feature_overrides(self) -> None:
        ...
        try:
            ...
            raise frappe.ValidationError(
                _(
                    "Feature Overrides must contain valid JSON. " "Original error: {0}"
                ).format(exc)
            ) from exc
        ...
        for feature_key, value in overrides.items():
            ...
            if not isinstance(value, (bool, dict)):
                raise frappe.ValidationError(
                    _(
                        "Override for feature {0} must be a boolean or nested configuration."
                    ).format(feature_key),
                )
