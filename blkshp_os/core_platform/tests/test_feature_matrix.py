...
class TestFeatureMatrix(FrappeTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.department = self._create_department(
            "Matrix Department", "MATRIX", self.company.name
        )
    ...
    def test_disabled_module_blocks_user_but_not_operations(self) -> None:
        ...
        try:
            ...
            standard_matrix = get_feature_matrix_for_user(
                user=self.standard_user, refresh=True
            )
            operations_matrix = get_feature_matrix_for_user(
                user=self.operations_user, refresh=True
            )

            standard_inventory = next(
                module
                for module in standard_matrix["modules"]
                if module["key"] == "inventory"
            )
            operations_inventory = next(
                module
                for module in operations_matrix["modules"]
                if module["key"] == "inventory"
            )
