...
def get_all_tenants() -> list[dict[str, Any]]:
    ...
    for company in companies:
        ...
        tenant_data = {
            ...
            "billing_frequency": (
                context.plan.billing_frequency if context.plan else None
            ),
            "base_price": context.plan.base_price if context.plan else None,
            "enabled_modules": enabled_modules,
            "module_count": len(enabled_modules),
            "has_overrides": (
                bool(context.plan and context.plan.default_feature_overrides)
                if context.plan
                else False
            ),
        }
...
def toggle_module(
    ...
    activation_name = frappe.db.get_value(
        "Module Activation", {"plan": plan_code, "module_key": module_key}, "name"
    )
...
def _log_admin_action(action: str, company: str, details: dict[str, Any]) -> None:
    ...
    try:
        ...
        frappe.log_error(
            f"Failed to log admin action: {e!s}", "Subscription Management"
        )
